...
export class DeviceDailyExceptionComponent extends TableComponent {
  ...
  override onComplete(): string {
    ...
    if (this.result.length == 0) {
      ...
      data.map((it: any) => {
        console.log(it["lng"], it["lat"]);
        if (it["lng"] != null && it["lat"] != null) {
          this.spMap.addMarker(it["lng"], it["lat"], it["SERIAL_NUMBER"]);
          this.FoundDevices.push(it)
        } else {
          it["Last_Location"] = ""
  ...
  downloadReport() {
    this.genCSV(this.notFoundDevices, "DevicesMissingLocation")
  }
  downloadFoundReport() {
    this.genCSV(this.FoundDevices, "DevicesLocation")
  }
