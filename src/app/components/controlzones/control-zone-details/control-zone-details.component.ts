...
import { radiusData } from '../../map/map.component';

@Component({
  selector: 'sp-control-zone-details',
...
export class ControlZoneDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    rd.id = this.details.id.toString();
    rd.latitude = this.details.lat;
    rd.longitude = this.details.lng;
    rd.color = this.details.color;
    rd.radius = this.details.radius;
